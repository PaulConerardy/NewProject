...
class DetectionRunner:
    def __init__(self, config_path='config.yaml', target_entities_path: str = None):
        """
        Initialise le Runner de Détection avec la configuration.
        
        Args:
            config_path (str): Chemin vers le fichier de configuration.
            target_entities_path (str, optional): Chemin vers le fichier parquet contenant la liste des entités à traiter.
        """
        # Initialiser Spark
        ...
        self.data_extractor = DataExtractor(config_path)
        
        # Charger la liste des entités cibles si spécifiée
        if target_entities_path:
            self.data_extractor.load_target_entities(target_entities_path)
        
        # Définir les chemins pour les fichiers de sortie
    ...
    def load_data(self):
        """
        Charger les données depuis la base de données Oracle via ServiceFlight.
        
        Returns:
            tuple: (entity_df, transactions_df, wires_df)
        """
        try:
            # Charger les données via l'extracteur
            entity_df = self.data_extractor.get_entities()
            transactions_df = self.data_extractor.get_transactions()
            wires_df = self.data_extractor.get_wires()
