...
class TransactionFeatureEngineer:
    ...
    def _create_amount_features(self, df):
        ...
        for window in ['1D', '7D', '30D']:
            ...
            for customer_id in df.index.get_level_values(0).unique():
                mask = df.index.get_level_values(0) == customer_id
    ...
    def _create_peer_groups(self, df):
        ...
        # Calculate tx_frequency with handling for infinite values
        time_diff = (df.groupby('customer_id')['timestamp'].transform('max') - 
                    df.groupby('customer_id')['timestamp'].transform('min')).dt.days
        
        # Handle zero time difference by setting to 1 day minimum
        time_diff = time_diff.replace(0, 1)
        
        df['tx_frequency'] = df.groupby('customer_id').size() / time_diff
